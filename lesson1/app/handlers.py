...
async def cmd_start(message: Message):
    ...
    await message.reply(f'Добро пожаловать в магазин компьютерной техники!', reply_markup = kb.main)
...
async def category(callback: CallbackQuery):
    ...
    await callback.message.answer(f'Название: {item_data.name}\nОписание: {item_data.description}\n'
                                  f'Цена: {item_data.price}$', reply_markup = await kb.items(callback.data.split('_')[1]))